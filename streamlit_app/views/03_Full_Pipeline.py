    ...
    if st.button("🚀 Exécuter le Pipeline Complet", use_container_width=True, type="primary"):
        ...
        with col1:
            ...
            <div class="glass-card" style="background:{b_classic};border:1px solid {c_classic}44;padding:1.5rem;height:100%">
                <div style="color:#6b7280;font-size:0.75rem;font-weight:700;text-transform:uppercase;margin-bottom:0.5rem">1. Modèle Classique (MLP)</div>
                <div style="font-size:2.2rem;font-weight:800;color:{c_classic};margin-bottom:0.2rem">{r_classic.upper()}</div>
                <div style="font-size:0.9rem;color:#6b7280;font-weight:500;">Probabilité: <strong>{classic_level.get('probability', 0):.1%}</strong></div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            <div class="glass-card" style="background:{b_hf};border:1px solid {c_hf}44;padding:1.5rem;height:100%">
                <div style="color:#6b7280;font-size:0.75rem;font-weight:700;text-transform:uppercase;margin-bottom:0.5rem">2. Zero-Shot NLP</div>
                <div style="font-size:1.6rem;line-height:1.2;font-weight:800;color:{c_hf};margin-bottom:0.2rem">{h_cat.title()}</div>
                <div style="font-size:0.9rem;color:#6b7280;margin-top:0.4rem;font-weight:500;">Confiance: <strong>{hf_result.get('confidence', 0):.1%}</strong></div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            ...
            <div class="glass-card" style="background:{b_llm};border:1px solid {c_llm}44;padding:1.5rem;height:100%">
                <div style="color:#6b7280;font-size:0.75rem;font-weight:700;text-transform:uppercase;margin-bottom:0.5rem">3. Synthèse LLM</div>
                <div style="font-size:2.2rem;font-weight:800;color:{c_llm};margin-bottom:0.2rem">{r_llm.upper()}</div>
                <div style="font-size:0.9rem;color:#6b7280;font-weight:500;">Urgence: <strong>{llm_result.get('urgency', '-').upper()}</strong></div>
            </div>
            """, unsafe_allow_html=True)
        ...
        <div class="glass-card" style="background:{b_final};border:2px solid {c_final};padding:2rem;display:flex;align-items:center;gap:2rem">
            <div style="text-align:center;min-width:160px;border-right:1px solid {c_final}44;padding-right:2rem">
                <p style="margin:0;font-size:0.85rem;text-transform:uppercase;font-weight:800;color:#6b7280">Consensus Final</p>
                <p style="margin:0;font-size:2.5rem;font-weight:800;color:{c_final}">{consensus.get('final_risk_level', '-').upper()}</p>
                <p style="margin:0;font-size:0.9rem;color:#6b7280;font-weight:600;">Accord: <strong>{consensus.get('agreement', '-').replace('_', ' ')}</strong></p>
            </div>
            <div style="flex-grow:1">
                <p style="margin:0;font-family:'Fraunces',serif;font-size:1.3rem;font-weight:700;color:#111827">Justification Croisée</p>
                <p style="margin:0.5rem 0 0;font-size:1.05rem;color:#374151;line-height:1.6">{consensus.get('cross_validation', '-')}</p>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="glass-card" style="background:var(--panel-solid);padding:2rem">
            <p style="color:#111827;font-size:1.1rem;line-height:1.7;margin-bottom:1.5rem">{llm_result.get('summary', '')}</p>
            <p style="font-weight:800;color:#6b7280;margin-bottom:0.5rem;text-transform:uppercase;font-size:0.85rem;letter-spacing:0.05em;">Recommandations</p>
            <ul style="color:#111827;margin:0;padding-left:1.5rem;font-size:1.05rem;line-height:1.6">
        ...
        <div class="glass-card" style="padding:4rem 2rem;text-align:center;margin-top:2.5rem">
            ...
            <p style="color:#6b7280;margin:1rem 0 0;font-size:1.15rem;line-height:1.6">
